...
from sklearn import preprocessing, metrics, discriminant_analysis
...
from classifier import computer_estimates
from classifier import lda_classifier
from classifier import qda_classifier
...
def lda_cls(dataset, idx_class):
    (x_train, x_test, y_train, y_test) = \
        train_test_split(dataset.iloc[:, 0:idx_class].values, dataset.iloc[:, idx_class].values, train_size=0.8)

    LDA_estimates_test,variance_estimate_test = computer_estimates.compute_estimates(x_test,y_test,classifier="LDA")
    predictions = lda_classifier.LDA_classifier(x_test,LDA_estimates_test,variance_estimate_test)
...
def qda_cls(dataset, idx_class):
    sh_df = shuffle(dataset)
    (x_train, x_test, y_train, y_test) = \
        train_test_split(sh_df.iloc[:, 0:idx_class].values, sh_df.iloc[:, idx_class].values, train_size=0.8)

    #QDA_estimates_test = computer_estimates.compute_estimates(x_test,y_test)
    #predictions = qda_classifier.QDA_classifier(x_test,QDA_estimates_test)
    ...
    accuracy = accuracy_score(y_test,predictions)
    #accuracy_s = accuracy_score(y_test,predicted_y)
...
def process_data(dt, idx_class, lb=True, selected_colums=[]):
    ...
    for n in range(19):
        lda_interactions.append(lda_cls(df, idx_class))
        qda_interactions.append(qda_cls(df, idx_class))

    print_interactions(lda_interactions, "LDA Classificador")
    print_interactions(qda_interactions, "QDA Classificador")
